...
| 2   | Integrations + OAuth-from-chat | `mcp/integrations`  | TBD    | open    | Integrations card + `/oauth-return` route + `apps/dev/src/routes/integrations.ts` shim + `vite.config` ngrok-allowed-hosts + `apps/dev/src/index.ts` FRONTEND_URL warning. Stacked on `mcp/scaffolding`. |
