...
| 10  | Actors + Events + Triggers   | `mcp/actors-triggers` | #388   | open    | Smaller cards. **Stacks on `mcp/objects-graph`** because the actors card uses `ContentFold` (introduced by slice 9). |
