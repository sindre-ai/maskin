...
export const tools = {
	...
	subscribe_session_logs: {
		description:
			'Subscribe to live log output from a container session (stdout, stderr, and system messages). The MCP server opens a long-lived SSE connection to /api/sessions/:id/logs/stream and pushes each log line to this client as a logging notification (logger="maskin/session-logs"). Use this to watch a session you spawned with create_session in real time, instead of polling get_session. Delivery format: { subscription_id, kind: "log", session_id, stream: "stdout"|"stderr"|"system", content } per line, then a final { kind: "done", session_id, status: "completed"|"failed"|"timeout" } when the session terminates — after which the subscription is auto-removed. If the stream ends before a terminal frame (e.g., transport error, auth revocation) the client still receives a { kind: "terminated", session_id, reason } close signal. For historical or already-completed sessions, use get_session with include_logs: true instead. Returns { subscription_id, session_id }.',
		inputSchema: z.object({
			workspace_id: optionalWorkspaceId,
