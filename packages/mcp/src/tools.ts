...
export const tools = {
	...
	// ─── LinkedIn (Unipile-backed, actor-scoped) ─────────────
	// Schemas are the frozen public contract the MCP host presents to the
	// agent — never change field names without a bet. The handlers registered
	// in server.ts proxy each call to the corresponding thin backend route
	// (apps/dev/src/routes/integrations-linkedin-unipile.ts), which does the
	// actor-scoped credential lookup, calls Unipile, and applies the
	// idempotency dedup + six-class error taxonomy. Requires the calling
	// actor to have connected LinkedIn via the `linkedin-unipile` provider
	// (Task 2 owns the connect flow).
	linkedin__send_message: {
		description:
			'Send a LinkedIn direct message to a recipient on behalf of the connected LinkedIn identity of the calling actor. Requires the actor to have connected LinkedIn via the linkedin-unipile integration. Returns the LinkedIn-side message id and the timestamp the send was accepted by LinkedIn (not delivery confirmation to the recipient inbox).',
		inputSchema: z.object({
			workspace_id: optionalWorkspaceId,
			recipient_urn: z
				.string()
				.min(1)
				.describe(
					'The LinkedIn URN of the recipient member, e.g. "urn:li:person:AbC123". Obtain from linkedin__list_conversations or from contact metadata. Do NOT pass a Unipile account_id here — this is the LinkedIn-native recipient identifier.',
				),
			body: z
				.string()
				.min(1)
		...
		outputSchema: z.object({
			...
			sent_at: z
				.string()
				.datetime()
				.describe('ISO-8601 timestamp when LinkedIn accepted the send.'),
			replayed: z
				.boolean()
				.describe(
					'True if this call was deduplicated via idempotency_key and no new send occurred.',
				),
		}),
	},
	linkedin__list_conversations: {
		description:
			"List the connected actor's LinkedIn conversations, newest first. Paginated via an opaque cursor. Use this to discover recipient_urns and thread_ids for follow-on send/reply calls.",
		inputSchema: z.object({
			workspace_id: optionalWorkspaceId,
			cursor: z
				.string()
				.optional()
				.describe("Opaque cursor from a prior response's next_cursor. Omit for the first page."),
			limit: z
				.number()
				.int()
				.min(1)
				.max(50)
				.default(25)
				.describe('Max conversations to return. Defaults to 25.'),
		}),
		outputSchema: z.object({
			conversations: z.array(
				z.object({
					thread_id: z
						.string()
						.describe('Stable identifier for the conversation; pass to linkedin__reply.'),
					participants: z.array(
						z.object({
							recipient_urn: z.string(),
			...
			next_cursor: z
				.string()
				.optional()
				.describe('Present if more results exist. Omit means end of list.'),
		}),
	},
	linkedin__reply: {
		description:
			'Reply in an existing LinkedIn conversation thread on behalf of the connected LinkedIn identity of the calling actor. Use thread_id from linkedin__list_conversations.',
		inputSchema: z.object({
			workspace_id: optionalWorkspaceId,
			thread_id: z.string().min(1).describe('Thread id from linkedin__list_conversations.'),
			body: z.string().min(1).max(8000).describe('Plain-text reply body. Max 8000 chars.'),
			idempotency_key: z
				.string()
				.min(1)
				.max(128)
				.describe(
					'See linkedin__send_message.idempotency_key. Recommended: "{thread_id}:{draft_id}".',
				),
		}),
		outputSchema: z.object({
			message_id: z.string(),
