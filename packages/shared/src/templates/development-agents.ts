...
- One **umbrella PR** (\`bet/<short-slug> → main\`) opens when the last task is merged into the bet branch. That single PR is the bet's review window against main, and the technical owner merges it at bet completion.
...
| Umbrella PR \`bet/<slug>\` → \`main\` | the technical owner (once at bet completion) |

**Never hold a task PR open waiting for the technical owner.** Task PRs merge into the bet branch as soon as CTO PASS is issued. the technical owner's only merge decision is the single umbrella PR at the end.
...
> Using shared-branch mode: \`bet/<short-slug>\` is cut off \`main\`. All task PRs target it, not \`main\`. Developer merges task PRs into the bet branch autonomously after CTO PASS. the technical owner merges one umbrella PR to \`main\` at bet completion.
...
- **Holding task PRs open waiting for the technical owner.** Task PRs merge into the bet branch as soon as CTO PASS is issued. the technical owner only touches the umbrella PR.
...
11. **Corroboration floor still met.** D2 sub-A ≥ 4 still holds (≥3 independent sources, at least one behavioural/analytics). If recency decay or de-duplication during shaping dropped sub-A below 4, this bet should not be auto-running — revert \`promotion_mode\` to \`human_approved\` and surface to the workspace owner.
...
If any of 8–11 fails on a bet with \`promotion_mode=auto\`, downgrade to \`human_approved\` and notify the workspace owner before continuing.
...
1. Council promote-door (or fast-track) creates the bet at \`define\` with \`promotion_mode\` set, \`informs\` edges to source insight(s), and the workspace owner @-mentioned in the digest.
...
3. @mention a founder (per \`maskin-voice\`, resolve via \`list_actors\`; strategy/UX decisions to the workspace owner, architecture to the technical owner — capability-holding is a strategy call, so default to the workspace owner) with a single question: *"Should we graduate this into a Commitment?"*
...
- **the workspace owner** → design, UX, business, strategy
- **the technical owner** → architecture, dev, PRs

Resolve both from \`list_actors\` (filter \`type: human\`); never hardcode an actor id.

If a topic spans both, pick the one the *decision* sits in, not the one the *context* sits in — a copy decision in an architecture diff still goes to the workspace owner; a perf fix on a design surface still goes to the technical owner. One mention per comment unless the decision genuinely requires both.
...
**Bad:**
> @owner — booking link sent 5 days ago (5/17), still no booking. Auto follow-up fires 5/24.
>
...
**Good:**
> Ahmad still hasn't booked, auto FU fires tomorrow. Task has a 3-slot nudge ready (Mon/Tue/Wed 10:00 or 14:00). Approve, edit, or kill. @owner
...
**Good:**
> ⚠ This bet touches anchor #5 (distribution/GTM) for the first time — Maskin selling its own subscription, not tooling for customers, so it's not a drift violation, but worth naming. Pre-PMF stage intensity usually weights #1/#2 over #5. Worth a sentence in the bet acknowledging it's deliberate. @owner
...
- Always @mention the relevant decision-maker — for design/arch proposals this is the Strategist; for bet/product decisions this is the workspace owner
