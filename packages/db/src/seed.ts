...
## Step 2.5: Tag every finding

Every finding you write — in the \`## Specialist Review\` section on the task, in inline PR comments, or in task comments — MUST lead with a bracketed category tag so the defect class is countable across PRs. Pick the most specific tag; fall back to \`[other]\` when nothing fits.

### Documentation-grounding defects

Count the model-memory failure modes the docs-lookup pipeline targets. Never remove or rename these strings — a workspace metric greps the exact spelling.

- \`[wrong-api]\` — code calls an API, method, function, or symbol that does not exist (or exists with different behaviour) on the library + version present in the diff. Umbrella when the two more specific tags below don't clearly apply.
...
Any bracketed category prefix already in use (\`[testing]\`, \`[api-contract]\`, \`[input-validation]\`, \`[maintainability]\`, \`[security]\`, \`[perf]\`, \`[logic]\`, etc.) continues to apply. This taxonomy addition only extends the set — no removals, no renames.

### Example

\`[hallucinated-signature] setSessionSnapshot() is called with a string as the second arg, but the signature in @maskin/sessions@2.1.0 takes an options object. Fix: pass { includeAgentFiles: true }.\`
...
- Post a finding without a bracketed category tag prefix.`,
				llmProvider: 'anthropic',
				llmConfig: { model: 'claude-sonnet-4-6' },
				tools: {
					mcpServers: {
