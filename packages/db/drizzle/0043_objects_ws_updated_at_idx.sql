...
-- updated_before / updated_after filters on list_objects so callers can
-- query directly for stale work instead of fetching the whole workspace and
-- filtering in JS. Without this index the planner falls back to a sequential
-- scan once a workspace grows past a few thousand rows.
