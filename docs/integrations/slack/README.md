...
| Search message content | **user** (`xoxp-`) | `slack_search_public`, `slack_search_public_and_private` |
...
The user token carries the Slack visibility of whoever installed the app, including their private channels and DMs. That is why it is confined to search and why the two search tools are registered **only** when a user token is present: a workspace whose install predates the `user_scope` grant sees no search tools at all, rather than tools that always fail.
