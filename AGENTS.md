...
## Onboarding a new user from Codex

If the user asks how to start this repo, start it, set it up, or similar — follow this flow exactly. The goal is to get them from clone → running workspace → MCP-connected → `get_started` in as few steps as possible, inside a SINGLE Codex session. Do NOT guess commands or URLs; read the dev server's own banner output.
...
- `apps/web/AGENTS.md` — detailed frontend guidance: product philosophy, design system, component conventions, routing, state management, SSE patterns, and styling rules

## Project Rules
- `.Codex/rules/frontend.md` — frontend component reuse, DRY, and consistency rules (shadcn/ui, Radix UI)
- `.Codex/rules/testing.md` — testing conventions for all test types (unit, integration, E2E, frontend)
- `.Codex/rules/pre-commit.md` — pre-commit checklist (lint, type-check, tests)
- `.Codex/rules/pr-merge.md` — PR merge checklist (up to date with main, lint, type-check, tests)
- `.Codex/rules/input-validation.md` — input validation requirements at system boundaries (HTTP params, env vars, DB triggers)
- `.Codex/rules/structural-verification.md` — file placement and build configuration verification checklist
...
- All external inputs validated at system boundaries — see `.Codex/rules/input-validation.md` for specifics
...
## Testing
- **Full conventions**: see `.Codex/rules/testing.md` for patterns, file locations, and examples
- **Pre-commit**: see `.Codex/rules/pre-commit.md` — always run lint, type-check, and tests before committing
