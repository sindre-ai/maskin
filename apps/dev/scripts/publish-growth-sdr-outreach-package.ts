// One-shot publisher for the SDR Outreach Loop catalog bundle.
//
// Snapshots Sales Rep, Sales Rep (Magnus), Rune Værk, and Sales Coach, plus
...
//   pnpm --filter @maskin/dev exec tsx scripts/publish-growth-sdr-outreach-package.ts
//
...
import {
	...
	GROWTH_SDR_OUTREACH_SOURCE_WORKSPACE_ID,
	GROWTH_SDR_OUTREACH_TRIGGER_IDS,
	actorSnapshot,
	skillSnapshot,
	triggerSnapshot,
} from '../src/lib/catalog-packages/growth-sdr-outreach-package'
import {
	getActorData,
	getSkillData,
	getTriggerData,
...
const SOURCE_WORKSPACE_ID =
	process.env.GROWTH_SDR_OUTREACH_SOURCE_WORKSPACE_ID ?? GROWTH_SDR_OUTREACH_SOURCE_WORKSPACE_ID
...
async function main(): Promise<void> {
	...
	const actorRows = GROWTH_SDR_OUTREACH_ACTOR_IDS.map(getActorData)
	const triggerRows = GROWTH_SDR_OUTREACH_TRIGGER_IDS.map(getTriggerData)
	const skillRows = GROWTH_SDR_OUTREACH_SKILL_IDS.map(getSkillData)
	...
	// installer workspace.
	const publishedActorIds = new Set<string>(GROWTH_SDR_OUTREACH_ACTOR_IDS)
	...
	const [existing] = await db
		...
		.where(eq(catalogPackages.slug, GROWTH_SDR_OUTREACH_PACKAGE.slug))
		.limit(1)

	if (existing && !FORCE) {
		console.error(
			`Package ${GROWTH_SDR_OUTREACH_PACKAGE.slug} is already published as ${existing.id} at v${existing.version}. Pass --force to delete and re-create.`,
		)
		process.exit(1)
	}
	...
	if (existing && FORCE) {
		...
		if (install) {
			console.error(
				`Cannot --force re-publish ${GROWTH_SDR_OUTREACH_PACKAGE.slug}: it has at least one active install (installed_packages.source_package_id = ${existing.id}). Deleting it would orphan those installs — publish a new version instead.`,
			)
			process.exit(1)
		}
	}
	...
	const inserted = await db.transaction(async (tx) => {
		...
		const [pkg] = await tx
			...
			.values({
				slug: GROWTH_SDR_OUTREACH_PACKAGE.slug,
				name: GROWTH_SDR_OUTREACH_PACKAGE.name,
				description: GROWTH_SDR_OUTREACH_PACKAGE.description,
				version: GROWTH_SDR_OUTREACH_PACKAGE.version,
				useCase: GROWTH_SDR_OUTREACH_PACKAGE.useCase,
			})
			.returning()
	...
	console.log(
		`Published ${GROWTH_SDR_OUTREACH_PACKAGE.slug} v${GROWTH_SDR_OUTREACH_PACKAGE.version} as ${inserted.pkg.id} (${actorRows.length} actors + ${triggerRows.length} triggers + ${skillRows.length} skills = ${inserted.itemCount} items).`,
	)
	process.exit(0)
}
