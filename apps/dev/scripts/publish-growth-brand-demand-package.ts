// One-shot publisher for the Brand & Demand Loop catalog bundle.
//
// Snapshots Influencer Manager, Demo Video Producer, Visual Designer, Search &
// AI Visibility Analyst, and Event Producer, plus every trigger they drive,
// from the checked-in data/growth-actors.json + data/growth-triggers.json
// (captured live from the Growth workspace) into a single `catalog_packages`
// row at version 1.0.0 plus one `catalog_package_items` row per element.
// Triggers carry the source `target_actor_id` inside the snapshot — the
// install path rewrites that against the install's `source_item_id` map.
//
// Run once against the target DB:
//   pnpm --filter @maskin/dev exec tsx scripts/publish-growth-brand-demand-package.ts
//
...
import {
	...
	GROWTH_BRAND_DEMAND_SOURCE_WORKSPACE_ID,
	GROWTH_BRAND_DEMAND_TRIGGER_IDS,
	actorSnapshot,
	skillSnapshot,
	triggerSnapshot,
} from '../src/lib/catalog-packages/growth-brand-demand-package'
import {
	getActorData,
	getSkillData,
	getTriggerData,
...
const SOURCE_WORKSPACE_ID =
	process.env.GROWTH_BRAND_DEMAND_SOURCE_WORKSPACE_ID ?? GROWTH_BRAND_DEMAND_SOURCE_WORKSPACE_ID
...
async function main(): Promise<void> {
	...
	const actorRows = GROWTH_BRAND_DEMAND_ACTOR_IDS.map(getActorData)
	const triggerRows = GROWTH_BRAND_DEMAND_TRIGGER_IDS.map(getTriggerData)
	const skillRows = GROWTH_BRAND_DEMAND_SKILL_IDS.map(getSkillData)
	...
	// installer workspace.
	const publishedActorIds = new Set<string>(GROWTH_BRAND_DEMAND_ACTOR_IDS)
	...
	const [existing] = await db
		...
		.where(eq(catalogPackages.slug, GROWTH_BRAND_DEMAND_PACKAGE.slug))
		.limit(1)

	if (existing && !FORCE) {
		console.error(
			`Package ${GROWTH_BRAND_DEMAND_PACKAGE.slug} is already published as ${existing.id} at v${existing.version}. Pass --force to delete and re-create.`,
		)
		process.exit(1)
	}
	...
	if (existing && FORCE) {
		...
		if (install) {
			console.error(
				`Cannot --force re-publish ${GROWTH_BRAND_DEMAND_PACKAGE.slug}: it has at least one active install (installed_packages.source_package_id = ${existing.id}). Deleting it would orphan those installs — publish a new version instead.`,
			)
			process.exit(1)
		}
	}
	...
	const inserted = await db.transaction(async (tx) => {
		...
		const [pkg] = await tx
			...
			.values({
				slug: GROWTH_BRAND_DEMAND_PACKAGE.slug,
				name: GROWTH_BRAND_DEMAND_PACKAGE.name,
				description: GROWTH_BRAND_DEMAND_PACKAGE.description,
				version: GROWTH_BRAND_DEMAND_PACKAGE.version,
				useCase: GROWTH_BRAND_DEMAND_PACKAGE.useCase,
			})
			.returning()
	...
	console.log(
		`Published ${GROWTH_BRAND_DEMAND_PACKAGE.slug} v${GROWTH_BRAND_DEMAND_PACKAGE.version} as ${inserted.pkg.id} (${actorRows.length} actors + ${triggerRows.length} triggers + ${skillRows.length} skills = ${inserted.itemCount} items).`,
	)
	process.exit(0)
}
