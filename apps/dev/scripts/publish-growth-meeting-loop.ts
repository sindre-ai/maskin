// One-shot publisher for the Meeting Loop marketplace bundle.
//
...
//   pnpm --filter @maskin/dev exec tsx scripts/publish-growth-meeting-loop.ts
//
...
import {
	...
	skillSnapshot,
	triggerSnapshot,
} from '../src/lib/marketplace-loops/growth-meeting-loop'
...
async function main(): Promise<void> {
	...
	const [existing] = await db
		...
		.where(eq(marketplaceLoops.slug, GROWTH_MEETING_LOOP.slug))
		.limit(1)

	if (existing && !FORCE) {
		console.error(
			`Loop ${GROWTH_MEETING_LOOP.slug} is already published as ${existing.id} at v${existing.version}. Pass --force to delete and re-create.`,
		)
		process.exit(1)
	}
	...
	if (existing && FORCE) {
		...
		if (install) {
			console.error(
				`Cannot --force re-publish ${GROWTH_MEETING_LOOP.slug}: it has at least one active install (installed_loops.source_loop_id = ${existing.id}). Deleting it would orphan those installs — publish a new version instead.`,
			)
			process.exit(1)
		}
	}
	...
	const inserted = await db.transaction(async (tx) => {
		...
		const [loop] = await tx
			...
			.values({
				slug: GROWTH_MEETING_LOOP.slug,
				name: GROWTH_MEETING_LOOP.name,
				description: GROWTH_MEETING_LOOP.description,
				version: GROWTH_MEETING_LOOP.version,
				useCase: GROWTH_MEETING_LOOP.useCase,
			})
			.returning()
	...
	console.log(
		`Published ${GROWTH_MEETING_LOOP.slug} v${GROWTH_MEETING_LOOP.version} as ${inserted.loop.id} (${actorRows.length} actors + ${triggerRows.length} triggers + ${skillRows.length} skills = ${inserted.itemCount} items).`,
	)
	process.exit(0)
}
