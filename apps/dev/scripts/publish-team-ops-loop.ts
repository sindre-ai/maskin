// One-shot publisher for the Team Ops & Retro Loop marketplace bundle.
//
// Snapshots Workspace Coach and Retro Knowledge Author, plus every trigger
// they drive, from the checked-in data/dev-actors.json +
// data/dev-triggers.json (captured live, since the local dev Postgres is
// empty) into a single `marketplace_loops` row at version 1.0.0 plus one
...
//   pnpm --filter @maskin/dev exec tsx scripts/publish-team-ops-loop.ts
//
...
import {
	...
	TEAM_OPS_LOOP,
	TEAM_OPS_SKILL_IDS,
	TEAM_OPS_SOURCE_WORKSPACE_ID,
	TEAM_OPS_TRIGGER_IDS,
	actorSnapshot,
	skillSnapshot,
	triggerSnapshot,
} from '../src/lib/marketplace-loops/team-ops-loop'

const SOURCE_WORKSPACE_ID = process.env.TEAM_OPS_SOURCE_WORKSPACE_ID ?? TEAM_OPS_SOURCE_WORKSPACE_ID
...
async function main(): Promise<void> {
	...
	const actorRows = TEAM_OPS_ACTOR_IDS.map(getActorData)
	const triggerRows = TEAM_OPS_TRIGGER_IDS.map(getTriggerData)
	const skillRows = TEAM_OPS_SKILL_IDS.map(getSkillData)
	...
	// installer workspace.
	const publishedActorIds = new Set<string>(TEAM_OPS_ACTOR_IDS)
	...
	const [existing] = await db
		...
		.where(eq(marketplaceLoops.slug, TEAM_OPS_LOOP.slug))
		.limit(1)

	if (existing && !FORCE) {
		console.error(
			`Loop ${TEAM_OPS_LOOP.slug} is already published as ${existing.id} at v${existing.version}. Pass --force to delete and re-create.`,
		)
		process.exit(1)
	}
	...
	if (existing && FORCE) {
		...
		if (install) {
			console.error(
				`Cannot --force re-publish ${TEAM_OPS_LOOP.slug}: it has at least one active install (installed_loops.source_loop_id = ${existing.id}). Deleting it would orphan those installs — publish a new version instead.`,
			)
			process.exit(1)
		}
	}
	...
	const inserted = await db.transaction(async (tx) => {
		...
		const [loop] = await tx
			...
			.values({
				slug: TEAM_OPS_LOOP.slug,
				name: TEAM_OPS_LOOP.name,
				description: TEAM_OPS_LOOP.description,
				version: TEAM_OPS_LOOP.version,
				useCase: TEAM_OPS_LOOP.useCase,
			})
			.returning()
	...
	console.log(
		`Published ${TEAM_OPS_LOOP.slug} v${TEAM_OPS_LOOP.version} as ${inserted.loop.id} (${actorRows.length} actors + ${triggerRows.length} triggers + ${skillRows.length} skills = ${inserted.itemCount} items).`,
	)
	process.exit(0)
}
