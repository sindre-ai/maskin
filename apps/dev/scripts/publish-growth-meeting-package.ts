// One-shot publisher for the Meeting Loop catalog bundle.
//
// Snapshots Meeting Briefer and Meeting Follow-up, plus every trigger they
// drive, from the checked-in data/growth-actors.json +
...
//   pnpm --filter @maskin/dev exec tsx scripts/publish-growth-meeting-package.ts
//
...
import {
	...
	GROWTH_MEETING_SOURCE_WORKSPACE_ID,
	GROWTH_MEETING_TRIGGER_IDS,
	actorSnapshot,
	skillSnapshot,
	triggerSnapshot,
} from '../src/lib/catalog-packages/growth-meeting-package'
import {
	getActorData,
	getSkillData,
	getTriggerData,
...
const SOURCE_WORKSPACE_ID =
	process.env.GROWTH_MEETING_SOURCE_WORKSPACE_ID ?? GROWTH_MEETING_SOURCE_WORKSPACE_ID
...
async function main(): Promise<void> {
	...
	const actorRows = GROWTH_MEETING_ACTOR_IDS.map(getActorData)
	const triggerRows = GROWTH_MEETING_TRIGGER_IDS.map(getTriggerData)
	const skillRows = GROWTH_MEETING_SKILL_IDS.map(getSkillData)
	...
	// installer workspace.
	const publishedActorIds = new Set<string>(GROWTH_MEETING_ACTOR_IDS)
	...
	const [existing] = await db
		...
		.where(eq(catalogPackages.slug, GROWTH_MEETING_PACKAGE.slug))
		.limit(1)

	if (existing && !FORCE) {
		console.error(
			`Package ${GROWTH_MEETING_PACKAGE.slug} is already published as ${existing.id} at v${existing.version}. Pass --force to delete and re-create.`,
		)
		process.exit(1)
	}
	...
	if (existing && FORCE) {
		...
		if (install) {
			console.error(
				`Cannot --force re-publish ${GROWTH_MEETING_PACKAGE.slug}: it has at least one active install (installed_packages.source_package_id = ${existing.id}). Deleting it would orphan those installs — publish a new version instead.`,
			)
			process.exit(1)
		}
	}
	...
	const inserted = await db.transaction(async (tx) => {
		...
		const [pkg] = await tx
			...
			.values({
				slug: GROWTH_MEETING_PACKAGE.slug,
				name: GROWTH_MEETING_PACKAGE.name,
				description: GROWTH_MEETING_PACKAGE.description,
				version: GROWTH_MEETING_PACKAGE.version,
				useCase: GROWTH_MEETING_PACKAGE.useCase,
			})
			.returning()
	...
	console.log(
		`Published ${GROWTH_MEETING_PACKAGE.slug} v${GROWTH_MEETING_PACKAGE.version} as ${inserted.pkg.id} (${actorRows.length} actors + ${triggerRows.length} triggers + ${skillRows.length} skills = ${inserted.itemCount} items).`,
	)
	process.exit(0)
}
