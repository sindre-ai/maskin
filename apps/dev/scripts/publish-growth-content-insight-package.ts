// One-shot publisher for the Content & Insight Loop catalog bundle.
//
// Snapshots Content Writer, Insight Harvester, and Insights Synthesizer, plus
...
//   pnpm --filter @maskin/dev exec tsx scripts/publish-growth-content-insight-package.ts
//
...
import {
	...
	GROWTH_CONTENT_INSIGHT_SOURCE_WORKSPACE_ID,
	GROWTH_CONTENT_INSIGHT_TRIGGER_IDS,
	actorSnapshot,
	skillSnapshot,
	triggerSnapshot,
} from '../src/lib/catalog-packages/growth-content-insight-package'
import {
	getActorData,
	getSkillData,
	getTriggerData,
...
const SOURCE_WORKSPACE_ID =
	process.env.GROWTH_CONTENT_INSIGHT_SOURCE_WORKSPACE_ID ??
	GROWTH_CONTENT_INSIGHT_SOURCE_WORKSPACE_ID
...
async function main(): Promise<void> {
	...
	const actorRows = GROWTH_CONTENT_INSIGHT_ACTOR_IDS.map(getActorData)
	const triggerRows = GROWTH_CONTENT_INSIGHT_TRIGGER_IDS.map(getTriggerData)
	const skillRows = GROWTH_CONTENT_INSIGHT_SKILL_IDS.map(getSkillData)
	...
	// installer workspace.
	const publishedActorIds = new Set<string>(GROWTH_CONTENT_INSIGHT_ACTOR_IDS)
	...
	const [existing] = await db
		...
		.where(eq(catalogPackages.slug, GROWTH_CONTENT_INSIGHT_PACKAGE.slug))
		.limit(1)

	if (existing && !FORCE) {
		console.error(
			`Package ${GROWTH_CONTENT_INSIGHT_PACKAGE.slug} is already published as ${existing.id} at v${existing.version}. Pass --force to delete and re-create.`,
		)
		process.exit(1)
	}
	...
	if (existing && FORCE) {
		...
		if (install) {
			console.error(
				`Cannot --force re-publish ${GROWTH_CONTENT_INSIGHT_PACKAGE.slug}: it has at least one active install (installed_packages.source_package_id = ${existing.id}). Deleting it would orphan those installs — publish a new version instead.`,
			)
			process.exit(1)
		}
	}
	...
	const inserted = await db.transaction(async (tx) => {
		...
		const [pkg] = await tx
			...
			.values({
				slug: GROWTH_CONTENT_INSIGHT_PACKAGE.slug,
				name: GROWTH_CONTENT_INSIGHT_PACKAGE.name,
				description: GROWTH_CONTENT_INSIGHT_PACKAGE.description,
				version: GROWTH_CONTENT_INSIGHT_PACKAGE.version,
				useCase: GROWTH_CONTENT_INSIGHT_PACKAGE.useCase,
			})
			.returning()
	...
	console.log(
		`Published ${GROWTH_CONTENT_INSIGHT_PACKAGE.slug} v${GROWTH_CONTENT_INSIGHT_PACKAGE.version} as ${inserted.pkg.id} (${actorRows.length} actors + ${triggerRows.length} triggers + ${skillRows.length} skills = ${inserted.itemCount} items).`,
	)
	process.exit(0)
}
