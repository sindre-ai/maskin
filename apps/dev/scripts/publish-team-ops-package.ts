// One-shot publisher for the Team Ops & Retro Loop catalog bundle.
//
...
//   pnpm --filter @maskin/dev exec tsx scripts/publish-team-ops-package.ts
//
...
import {
	...
	skillSnapshot,
	triggerSnapshot,
} from '../src/lib/catalog-packages/team-ops-package'
...
async function main(): Promise<void> {
	...
	const [existing] = await db
		...
		.where(eq(catalogPackages.slug, TEAM_OPS_PACKAGE.slug))
		.limit(1)

	if (existing && !FORCE) {
		console.error(
			`Package ${TEAM_OPS_PACKAGE.slug} is already published as ${existing.id} at v${existing.version}. Pass --force to delete and re-create.`,
		)
		process.exit(1)
	}
	...
	if (existing && FORCE) {
		...
		if (install) {
			console.error(
				`Cannot --force re-publish ${TEAM_OPS_PACKAGE.slug}: it has at least one active install (installed_packages.source_package_id = ${existing.id}). Deleting it would orphan those installs — publish a new version instead.`,
			)
			process.exit(1)
		}
	}
	...
	const inserted = await db.transaction(async (tx) => {
		...
		const [pkg] = await tx
			...
			.values({
				slug: TEAM_OPS_PACKAGE.slug,
				name: TEAM_OPS_PACKAGE.name,
				description: TEAM_OPS_PACKAGE.description,
				version: TEAM_OPS_PACKAGE.version,
				useCase: TEAM_OPS_PACKAGE.useCase,
			})
			.returning()
	...
	console.log(
		`Published ${TEAM_OPS_PACKAGE.slug} v${TEAM_OPS_PACKAGE.version} as ${inserted.pkg.id} (${actorRows.length} actors + ${triggerRows.length} triggers + ${skillRows.length} skills = ${inserted.itemCount} items).`,
	)
	process.exit(0)
}
