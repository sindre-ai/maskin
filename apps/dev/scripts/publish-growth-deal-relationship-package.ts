// One-shot publisher for the Deal & Relationship Loop catalog bundle.
//
...
//   pnpm --filter @maskin/dev exec tsx scripts/publish-growth-deal-relationship-package.ts
//
...
import {
	...
	skillSnapshot,
	triggerSnapshot,
} from '../src/lib/catalog-packages/growth-deal-relationship-package'
import {
	getActorData,
	getSkillData,
	getTriggerData,
...
async function main(): Promise<void> {
	...
	const [existing] = await db
		...
		.where(eq(catalogPackages.slug, GROWTH_DEAL_RELATIONSHIP_PACKAGE.slug))
		.limit(1)

	if (existing && !FORCE) {
		console.error(
			`Package ${GROWTH_DEAL_RELATIONSHIP_PACKAGE.slug} is already published as ${existing.id} at v${existing.version}. Pass --force to delete and re-create.`,
		)
		process.exit(1)
	}
	...
	if (existing && FORCE) {
		...
		if (install) {
			console.error(
				`Cannot --force re-publish ${GROWTH_DEAL_RELATIONSHIP_PACKAGE.slug}: it has at least one active install (installed_packages.source_package_id = ${existing.id}). Deleting it would orphan those installs — publish a new version instead.`,
			)
			process.exit(1)
		}
	}
	...
	const inserted = await db.transaction(async (tx) => {
		...
		const [pkg] = await tx
			...
			.values({
				slug: GROWTH_DEAL_RELATIONSHIP_PACKAGE.slug,
				name: GROWTH_DEAL_RELATIONSHIP_PACKAGE.name,
				description: GROWTH_DEAL_RELATIONSHIP_PACKAGE.description,
				version: GROWTH_DEAL_RELATIONSHIP_PACKAGE.version,
				useCase: GROWTH_DEAL_RELATIONSHIP_PACKAGE.useCase,
			})
			.returning()
	...
	console.log(
		`Published ${GROWTH_DEAL_RELATIONSHIP_PACKAGE.slug} v${GROWTH_DEAL_RELATIONSHIP_PACKAGE.version} as ${inserted.pkg.id} (${actorRows.length} actors + ${triggerRows.length} triggers + ${skillRows.length} skills = ${inserted.itemCount} items).`,
	)
	process.exit(0)
}
