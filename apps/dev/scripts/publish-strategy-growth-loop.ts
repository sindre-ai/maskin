// One-shot publisher for the Strategy & Growth Loop marketplace bundle.
//
// Snapshots Strategist, Product Analyst, Product Marketer, and Product Pricing
// Specialist, plus every trigger they drive, from the checked-in
// data/dev-actors.json + data/dev-triggers.json (captured live, since the
// local dev Postgres is empty) into a single `marketplace_loops` row at
// version 1.0.0 plus one `marketplace_loop_items` row per element. Triggers
...
//   pnpm --filter @maskin/dev exec tsx scripts/publish-strategy-growth-loop.ts
//
...
import {
	...
	STRATEGY_GROWTH_SOURCE_WORKSPACE_ID,
	STRATEGY_GROWTH_TRIGGER_IDS,
	actorSnapshot,
	skillSnapshot,
	triggerSnapshot,
} from '../src/lib/marketplace-loops/strategy-growth-loop'

const SOURCE_WORKSPACE_ID =
	process.env.STRATEGY_GROWTH_SOURCE_WORKSPACE_ID ?? STRATEGY_GROWTH_SOURCE_WORKSPACE_ID
...
async function main(): Promise<void> {
	...
	const actorRows = STRATEGY_GROWTH_ACTOR_IDS.map(getActorData)
	const triggerRows = STRATEGY_GROWTH_TRIGGER_IDS.map(getTriggerData)
	const skillRows = STRATEGY_GROWTH_SKILL_IDS.map(getSkillData)
	...
	// installer workspace.
	const publishedActorIds = new Set<string>(STRATEGY_GROWTH_ACTOR_IDS)
	...
	const [existing] = await db
		...
		.where(eq(marketplaceLoops.slug, STRATEGY_GROWTH_LOOP.slug))
		.limit(1)

	if (existing && !FORCE) {
		console.error(
			`Loop ${STRATEGY_GROWTH_LOOP.slug} is already published as ${existing.id} at v${existing.version}. Pass --force to delete and re-create.`,
		)
		process.exit(1)
	}
	...
	if (existing && FORCE) {
		...
		if (install) {
			console.error(
				`Cannot --force re-publish ${STRATEGY_GROWTH_LOOP.slug}: it has at least one active install (installed_loops.source_loop_id = ${existing.id}). Deleting it would orphan those installs — publish a new version instead.`,
			)
			process.exit(1)
		}
	}
	...
	const inserted = await db.transaction(async (tx) => {
		...
		const [loop] = await tx
			...
			.values({
				slug: STRATEGY_GROWTH_LOOP.slug,
				name: STRATEGY_GROWTH_LOOP.name,
				description: STRATEGY_GROWTH_LOOP.description,
				version: STRATEGY_GROWTH_LOOP.version,
				useCase: STRATEGY_GROWTH_LOOP.useCase,
			})
			.returning()
	...
	console.log(
		`Published ${STRATEGY_GROWTH_LOOP.slug} v${STRATEGY_GROWTH_LOOP.version} as ${inserted.loop.id} (${actorRows.length} actors + ${triggerRows.length} triggers + ${skillRows.length} skills = ${inserted.itemCount} items).`,
	)
	process.exit(0)
}
