// One-shot publisher for the Strategy & Growth Loop catalog bundle.
//
...
//   pnpm --filter @maskin/dev exec tsx scripts/publish-strategy-growth-package.ts
//
...
import {
	...
	skillSnapshot,
	triggerSnapshot,
} from '../src/lib/catalog-packages/strategy-growth-package'
...
async function main(): Promise<void> {
	...
	const [existing] = await db
		...
		.where(eq(catalogPackages.slug, STRATEGY_GROWTH_PACKAGE.slug))
		.limit(1)

	if (existing && !FORCE) {
		console.error(
			`Package ${STRATEGY_GROWTH_PACKAGE.slug} is already published as ${existing.id} at v${existing.version}. Pass --force to delete and re-create.`,
		)
		process.exit(1)
	}
	...
	if (existing && FORCE) {
		...
		if (install) {
			console.error(
				`Cannot --force re-publish ${STRATEGY_GROWTH_PACKAGE.slug}: it has at least one active install (installed_packages.source_package_id = ${existing.id}). Deleting it would orphan those installs — publish a new version instead.`,
			)
			process.exit(1)
		}
	}
	...
	const inserted = await db.transaction(async (tx) => {
		...
		const [pkg] = await tx
			...
			.values({
				slug: STRATEGY_GROWTH_PACKAGE.slug,
				name: STRATEGY_GROWTH_PACKAGE.name,
				description: STRATEGY_GROWTH_PACKAGE.description,
				version: STRATEGY_GROWTH_PACKAGE.version,
				useCase: STRATEGY_GROWTH_PACKAGE.useCase,
			})
			.returning()
	...
	console.log(
		`Published ${STRATEGY_GROWTH_PACKAGE.slug} v${STRATEGY_GROWTH_PACKAGE.version} as ${inserted.pkg.id} (${actorRows.length} actors + ${triggerRows.length} triggers + ${skillRows.length} skills = ${inserted.itemCount} items).`,
	)
	process.exit(0)
}
