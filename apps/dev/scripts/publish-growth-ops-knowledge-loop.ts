// One-shot publisher for the Ops & Knowledge Loop marketplace bundle.
//
// Snapshots Knowledge Author, Workspace Coach, Workspace Driver, and Chief of
// Staff, plus every trigger they drive, from the checked-in
// data/growth-actors.json + data/growth-triggers.json (captured live from the
// Growth workspace) into a single `marketplace_loops` row at version 1.0.0
// plus one `marketplace_loop_items` row per element. Triggers carry the source
...
//   pnpm --filter @maskin/dev exec tsx scripts/publish-growth-ops-knowledge-loop.ts
//
...
import {
	...
	GROWTH_OPS_KNOWLEDGE_SOURCE_WORKSPACE_ID,
	GROWTH_OPS_KNOWLEDGE_TRIGGER_IDS,
	actorSnapshot,
	skillSnapshot,
	triggerSnapshot,
} from '../src/lib/marketplace-loops/growth-ops-knowledge-loop'
import { getActorData, getSkillData, getTriggerData } from '../src/lib/marketplace-loops/loop-data'

const SOURCE_WORKSPACE_ID =
	process.env.GROWTH_OPS_KNOWLEDGE_SOURCE_WORKSPACE_ID ?? GROWTH_OPS_KNOWLEDGE_SOURCE_WORKSPACE_ID
...
async function main(): Promise<void> {
	...
	const actorRows = GROWTH_OPS_KNOWLEDGE_ACTOR_IDS.map(getActorData)
	const triggerRows = GROWTH_OPS_KNOWLEDGE_TRIGGER_IDS.map(getTriggerData)
	const skillRows = GROWTH_OPS_KNOWLEDGE_SKILL_IDS.map(getSkillData)
	...
	// installer workspace.
	const publishedActorIds = new Set<string>(GROWTH_OPS_KNOWLEDGE_ACTOR_IDS)
	...
	const [existing] = await db
		...
		.where(eq(marketplaceLoops.slug, GROWTH_OPS_KNOWLEDGE_LOOP.slug))
		.limit(1)

	if (existing && !FORCE) {
		console.error(
			`Loop ${GROWTH_OPS_KNOWLEDGE_LOOP.slug} is already published as ${existing.id} at v${existing.version}. Pass --force to delete and re-create.`,
		)
		process.exit(1)
	}
	...
	if (existing && FORCE) {
		...
		if (install) {
			console.error(
				`Cannot --force re-publish ${GROWTH_OPS_KNOWLEDGE_LOOP.slug}: it has at least one active install (installed_loops.source_loop_id = ${existing.id}). Deleting it would orphan those installs — publish a new version instead.`,
			)
			process.exit(1)
		}
	}
	...
	const inserted = await db.transaction(async (tx) => {
		...
		const [loop] = await tx
			...
			.values({
				slug: GROWTH_OPS_KNOWLEDGE_LOOP.slug,
				name: GROWTH_OPS_KNOWLEDGE_LOOP.name,
				description: GROWTH_OPS_KNOWLEDGE_LOOP.description,
				version: GROWTH_OPS_KNOWLEDGE_LOOP.version,
				useCase: GROWTH_OPS_KNOWLEDGE_LOOP.useCase,
			})
			.returning()
	...
	console.log(
		`Published ${GROWTH_OPS_KNOWLEDGE_LOOP.slug} v${GROWTH_OPS_KNOWLEDGE_LOOP.version} as ${inserted.loop.id} (${actorRows.length} actors + ${triggerRows.length} triggers + ${skillRows.length} skills = ${inserted.itemCount} items).`,
	)
	process.exit(0)
}
