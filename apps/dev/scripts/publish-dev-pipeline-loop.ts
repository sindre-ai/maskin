// One-shot publisher for the Build & Ship Loop marketplace bundle.
//
...
//   pnpm --filter @maskin/dev exec tsx scripts/publish-dev-pipeline-loop.ts
//
...
import {
	...
	skillSnapshot,
	triggerSnapshot,
} from '../src/lib/marketplace-loops/dev-pipeline-loop'
...
async function main(): Promise<void> {
	...
	const [existing] = await db
		...
		.where(eq(marketplaceLoops.slug, DEV_PIPELINE_LOOP.slug))
		.limit(1)

	if (existing && !FORCE) {
		console.error(
			`Loop ${DEV_PIPELINE_LOOP.slug} is already published as ${existing.id} at v${existing.version}. Pass --force to delete and re-create.`,
		)
		process.exit(1)
	}
	...
	if (existing && FORCE) {
		...
		if (install) {
			console.error(
				`Cannot --force re-publish ${DEV_PIPELINE_LOOP.slug}: it has at least one active install (installed_loops.source_loop_id = ${existing.id}). Deleting it would orphan those installs — publish a new version instead.`,
			)
			process.exit(1)
		}
	}
	...
	const inserted = await db.transaction(async (tx) => {
		...
		const [loop] = await tx
			...
			.values({
				slug: DEV_PIPELINE_LOOP.slug,
				name: DEV_PIPELINE_LOOP.name,
				description: DEV_PIPELINE_LOOP.description,
				version: DEV_PIPELINE_LOOP.version,
				useCase: DEV_PIPELINE_LOOP.useCase,
			})
			.returning()
	...
	console.log(
		`Published ${DEV_PIPELINE_LOOP.slug} v${DEV_PIPELINE_LOOP.version} as ${inserted.loop.id} (${actorRows.length} actors + ${triggerRows.length} triggers + ${skillRows.length} skills = ${inserted.itemCount} items).`,
	)
	process.exit(0)
}
