// One-shot publisher for the Ops & Knowledge Loop catalog bundle.
//
...
// Growth workspace) into a single `catalog_packages` row at version 1.0.0
...
//   pnpm --filter @maskin/dev exec tsx scripts/publish-growth-ops-knowledge-package.ts
//
...
import {
	...
	skillSnapshot,
	triggerSnapshot,
} from '../src/lib/catalog-packages/growth-ops-knowledge-package'
import {
	getActorData,
	getSkillData,
	getTriggerData,
...
async function main(): Promise<void> {
	...
	const [existing] = await db
		...
		.where(eq(catalogPackages.slug, GROWTH_OPS_KNOWLEDGE_PACKAGE.slug))
		.limit(1)

	if (existing && !FORCE) {
		console.error(
			`Package ${GROWTH_OPS_KNOWLEDGE_PACKAGE.slug} is already published as ${existing.id} at v${existing.version}. Pass --force to delete and re-create.`,
		)
		process.exit(1)
	}
	...
	if (existing && FORCE) {
		...
		if (install) {
			console.error(
				`Cannot --force re-publish ${GROWTH_OPS_KNOWLEDGE_PACKAGE.slug}: it has at least one active install (installed_packages.source_package_id = ${existing.id}). Deleting it would orphan those installs — publish a new version instead.`,
			)
			process.exit(1)
		}
	}
	...
	const inserted = await db.transaction(async (tx) => {
		...
		const [pkg] = await tx
			...
			.values({
				slug: GROWTH_OPS_KNOWLEDGE_PACKAGE.slug,
				name: GROWTH_OPS_KNOWLEDGE_PACKAGE.name,
				description: GROWTH_OPS_KNOWLEDGE_PACKAGE.description,
				version: GROWTH_OPS_KNOWLEDGE_PACKAGE.version,
				useCase: GROWTH_OPS_KNOWLEDGE_PACKAGE.useCase,
			})
			.returning()
	...
	console.log(
		`Published ${GROWTH_OPS_KNOWLEDGE_PACKAGE.slug} v${GROWTH_OPS_KNOWLEDGE_PACKAGE.version} as ${inserted.pkg.id} (${actorRows.length} actors + ${triggerRows.length} triggers + ${skillRows.length} skills = ${inserted.itemCount} items).`,
	)
	process.exit(0)
}
