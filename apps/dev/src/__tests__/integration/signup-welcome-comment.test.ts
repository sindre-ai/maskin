...
describe('Signup welcome comment — Chief of Staff comments, Researcher gets spawned', () => {
	...
	it('posts a Chief-of-Staff comment mentioning Researcher and the signed-up human, and spawns a Researcher session', async () => {
		...
		// Researcher IS a real mention — that is what makes it a thread
		// participant (`routes/events.ts` reads prior `data.mentions` to decide
		// who counts as one), so a later human reply reaches it. Only its
		// generic CommentDispatcher session is suppressed, since
		// signup-welcome wires a bespoke one below.
