...
import {
	...
	skillSnapshot,
	triggerSnapshot,
} from '../../../lib/marketplace-loops/team-ops-loop'
...
describe('Team Ops & Retro Loop definition', () => {
	it('uses the correct slug and metadata', () => {
		expect(TEAM_OPS_LOOP.slug).toBe('team-ops-retro-loop')
		expect(TEAM_OPS_LOOP.name).toBe('Team Ops & Retro Loop')
		expect(TEAM_OPS_LOOP.useCase).toBe('Operations')
		expect(TEAM_OPS_LOOP.version).toBe('1.0.0')
