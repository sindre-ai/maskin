...
import {
	...
	skillSnapshot,
	triggerSnapshot,
} from '../../../lib/marketplace-loops/strategy-growth-loop'
...
describe('Strategy & Growth Loop definition', () => {
	it('uses the correct slug and metadata', () => {
		expect(STRATEGY_GROWTH_LOOP.slug).toBe('strategy-growth-loop')
		expect(STRATEGY_GROWTH_LOOP.name).toBe('Strategy & Growth Loop')
		expect(STRATEGY_GROWTH_LOOP.useCase).toBe('Growth')
		expect(STRATEGY_GROWTH_LOOP.version).toBe('1.0.0')
		expect(STRATEGY_GROWTH_LOOP.description.length).toBeGreaterThan(0)
	})
