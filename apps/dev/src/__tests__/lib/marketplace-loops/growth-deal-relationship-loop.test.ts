...
} from '../../../lib/marketplace-loops/growth-deal-relationship-loop'

describe('Deal & Relationship Loop definition', () => {
	it('uses the correct slug and metadata', () => {
		expect(GROWTH_DEAL_RELATIONSHIP_LOOP.slug).toBe('deal-relationship-loop')
		expect(GROWTH_DEAL_RELATIONSHIP_LOOP.name).toBe('Deal & Relationship Loop')
		expect(GROWTH_DEAL_RELATIONSHIP_LOOP.useCase).toBe('Sales')
		expect(GROWTH_DEAL_RELATIONSHIP_LOOP.version).toBe('1.0.0')
		expect(GROWTH_DEAL_RELATIONSHIP_LOOP.description.length).toBeGreaterThan(0)
	})
