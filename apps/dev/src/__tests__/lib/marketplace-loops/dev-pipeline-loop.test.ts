...
import {
	...
	skillSnapshot,
	triggerSnapshot,
} from '../../../lib/marketplace-loops/dev-pipeline-loop'
...
describe('Build & Ship Loop definition', () => {
	it('uses the correct slug and metadata', () => {
		expect(DEV_PIPELINE_LOOP.slug).toBe('build-ship-loop')
		expect(DEV_PIPELINE_LOOP.name).toBe('Build & Ship Loop')
		expect(DEV_PIPELINE_LOOP.useCase).toBe('Development')
		expect(DEV_PIPELINE_LOOP.version).toBe('1.0.0')
		expect(DEV_PIPELINE_LOOP.description.length).toBeGreaterThan(0)
	})
