...
} from '../../../lib/marketplace-loops/growth-lead-gen-loop'

describe('Lead Gen & Qualification Loop definition', () => {
	it('uses the correct slug and metadata', () => {
		expect(GROWTH_LEAD_GEN_LOOP.slug).toBe('lead-gen-qualification-loop')
		expect(GROWTH_LEAD_GEN_LOOP.name).toBe('Lead Gen & Qualification Loop')
		expect(GROWTH_LEAD_GEN_LOOP.useCase).toBe('Sales')
		expect(GROWTH_LEAD_GEN_LOOP.version).toBe('1.0.0')
		expect(GROWTH_LEAD_GEN_LOOP.description.length).toBeGreaterThan(0)
	})
