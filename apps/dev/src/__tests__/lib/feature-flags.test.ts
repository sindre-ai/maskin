...
describe('parseFeatureFlagConfig', () => {
	...
	it('trims whitespace and drops empty entries', () => {
		const c = config({ FF_TESTER_FEATURES: ' new-design ,, , other-flag ' })
		expect([...c.testerFlags]).toEqual(['new-design', 'other-flag'])
	})
...
describe('resolveFlags', () => {
	it('is false for every flag when the env is empty', () => {
		expect(resolveFlags(TESTER, config({}))).toEqual({ [FLAGS.NEW_DESIGN]: false })
	})
	...
	it('matches tester actor ids case-insensitively in both directions', () => {
		...
		expect(resolveFlags(TESTER, c)[FLAGS.NEW_DESIGN]).toBe(true)
		expect(resolveFlags(TESTER.toUpperCase(), c)[FLAGS.NEW_DESIGN]).toBe(true)
	})
	...
	it('ignores a legacy FF_EVERYONE var entirely', () => {
		const c = config({ FF_EVERYONE: FLAGS.NEW_DESIGN })
		expect(resolveFlags(NON_TESTER, c)[FLAGS.NEW_DESIGN]).toBe(false)
		expect(resolveFlags(TESTER, c)[FLAGS.NEW_DESIGN]).toBe(false)
	})
	...
	it('ignores unknown flag ids in the env and never returns them', () => {
		...
		const resolved = resolveFlags(TESTER, c)
		expect(resolved).toEqual({ [FLAGS.NEW_DESIGN]: false })
	...
	it('does not treat a whitespace-only actor id as an tester match', () => {
		const c = config({ FF_TESTER_FEATURES: FLAGS.NEW_DESIGN, FF_TESTER_ACTOR_IDS: ' , ' })
		expect(resolveFlags('   ', c)[FLAGS.NEW_DESIGN]).toBe(false)
	})
})
