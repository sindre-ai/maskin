...
describe('maybeBootstrapDev', () => {
	...
	it('creates actor + workspace + membership when DB has no credentials — no default agents seeded', async () => {
		process.env.NODE_ENV = 'development'
		process.env.MASKIN_AUTO_BOOTSTRAP = 'true'
		...
		expect(insertedNames).not.toContain('Chief of Staff')
		expect(calls.updates).toEqual([])
	})

	it('throws if workspace creation fails so the transaction rolls back', async () => {
		process.env.NODE_ENV = 'development'
		process.env.MASKIN_AUTO_BOOTSTRAP = 'true'
		const { db, mockResults } = createTestContext()
		...
		mockResults.insertQueue = [
			[{ id: 'actor-1', name: 'You', email: 'dev@local' }],
			[], // workspace insert returns empty → should throw
		]

		await expect(maybeBootstrapDev(db)).rejects.toThrow(/failed to create workspace/)
	})
})
