...
} from '../../../lib/catalog-packages/growth-brand-demand-package'

describe('Brand & Demand Loop package definition', () => {
	it('uses the correct slug and metadata', () => {
		expect(GROWTH_BRAND_DEMAND_PACKAGE.slug).toBe('brand-demand-loop')
		expect(GROWTH_BRAND_DEMAND_PACKAGE.name).toBe('Brand & Demand Loop')
		expect(GROWTH_BRAND_DEMAND_PACKAGE.useCase).toBe('Marketing')
		expect(GROWTH_BRAND_DEMAND_PACKAGE.version).toBe('1.0.0')
		expect(GROWTH_BRAND_DEMAND_PACKAGE.description.length).toBeGreaterThan(0)
	})
