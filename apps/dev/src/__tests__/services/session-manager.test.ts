...
describe('SessionManager', () => {
	...
	describe('startSession() — VERCEL_AUTOMATION_BYPASS_SECRET plumbing', () => {
		afterEach(() => {
			vi.unstubAllEnvs()
		})

		function buildFlaggedFixtures(opts: {
			bet_qa_required?: boolean
			...
			const session = buildSession({
				...
				containerId: null,
				config: {
					...(opts.bet_qa_required !== undefined && { bet_qa_required: opts.bet_qa_required }),
		...
		it('injects VERCEL_AUTOMATION_BYPASS_SECRET when bet_qa_required=true and host env has it (AC-T2 Docker leg)', async () => {
			vi.stubEnv('VERCEL_AUTOMATION_BYPASS_SECRET', 'bypass-from-host')
			const session = buildFlaggedFixtures({ bet_qa_required: true })
			...
			expect(createArgs.env.VERCEL_AUTOMATION_BYPASS_SECRET).toBe('bypass-from-host')
		})

		it('does not inject the secret when bet_qa_required is absent (cost + blast-radius control)', async () => {
			vi.stubEnv('VERCEL_AUTOMATION_BYPASS_SECRET', 'bypass-from-host')
			const session = buildFlaggedFixtures({})
			...
			expect(createArgs.env.VERCEL_AUTOMATION_BYPASS_SECRET).toBeUndefined()
		})

		it('does not inject the secret when bet_qa_required=false', async () => {
			vi.stubEnv('VERCEL_AUTOMATION_BYPASS_SECRET', 'bypass-from-host')
			const session = buildFlaggedFixtures({ bet_qa_required: false })
		...
		it('skips injection when the flag is on but the host env is unset', async () => {
		...
		it('refuses to let a user override VERCEL_AUTOMATION_BYPASS_SECRET via session config', async () => {
			vi.stubEnv('VERCEL_AUTOMATION_BYPASS_SECRET', 'bypass-from-host')
			const session = buildFlaggedFixtures({
				bet_qa_required: true,
				env_vars: { VERCEL_AUTOMATION_BYPASS_SECRET: 'attacker-supplied' },
			})
			...
			// User value is dropped by the reserved-key filter; host value wins.
			expect(createArgs.env.VERCEL_AUTOMATION_BYPASS_SECRET).toBe('bypass-from-host')
		})
	})
