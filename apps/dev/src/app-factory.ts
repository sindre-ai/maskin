...
export function createApp(deps: AppDeps, options: CreateAppOptions = {}): OpenAPIHono<Env> {
	...
	//   - GET /api/internal/fleet-heartbeat: off-fleet Cloudflare Workers cron
	//     hits this to detect fleet silence — the whole point is that it can't
	//     depend on any credential path shared with the Claude fleet. It carries
	//     its own X-Heartbeat-Secret and enforces it in the handler.
