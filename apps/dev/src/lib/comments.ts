...
/**
 * Core comment-creation logic: insert the `commented` event and resolve the
 * comment's mentions. Shared by `POST /api/events` and any backend code that
 * needs to post a comment programmatically.
 *
...
export async function postComment(
	...
	return db.transaction(async (tx) => {
		...
		// unresolved ids back to the client rather than silently dropping them.
