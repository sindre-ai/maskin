...
import { MeetError, MeetToolError, classifyGoogleApiError, classifyGoogleError } from './errors'

/**
 * Google Meet HTTP clients.
 *
 * Two slices landed on this file in parallel on the bet and both are kept:
 *
 *  - `callGoogleApi` (read-path Task 3) — the thin wrapper the read tools and
 *    the Workspace Events watch/webhook wiring use. No retry: reads are
 *    idempotent and callers handle the normalized MeetToolError envelope.
 *  - `createDefaultGoogleMeetClient` (write-path Task 4) — the retry/backoff
 *    client the create_space + create_meet_backed_event tools use, where a
 *    transient 429/5xx must not surface to the agent. Interface-shaped so
 *    tests inject a fake.
 *
 * They share the same normalized error envelope (see errors.ts) but map to it
 * through their slice's classifier; both emit the identical wire shape.
 */
