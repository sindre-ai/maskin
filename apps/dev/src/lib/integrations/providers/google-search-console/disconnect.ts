...
/**
 * Revoke the Google OAuth grant for a Google Search Console integration
 * before the row flips to `revoked`. Wired via the `preDisconnect` provider
 * hook so the grant disappears from the user's Google account at the same
 * moment Maskin stops trusting the credentials — without this, the token
 * would still be usable until it expires naturally.
 *
...
export async function revokeGoogleSearchConsoleGrant(
	...
	try {
		...
		if (!token) {
			logger.warn('Google Search Console disconnect: no token to revoke', {
		...
		if (config.auth.type === 'oauth2') {
			...
			logger.info('Google Search Console grant revoked', {
				integrationId: ctx.integrationId,
			})
		}
	} catch (err) {
		logger.warn('Google Search Console revoke failed (continuing with disconnect)', {
			integrationId: ctx.integrationId,
