...
export function matchKeyFor(mapped: MappedRow, matchOn: string): string | undefined {
	if (matchOn === 'title') return normalizeMatchKey(mapped.title)
	return normalizeMatchKey(mapped.metadata[matchOn.slice('metadata.'.length)])
}

/** SQL for the same normalization `normalizeMatchKey` applies, over the stored column. */
function matchKeyExpr(matchOn: string): SQL<string> {
	if (matchOn === 'title') return sql<string>`lower(btrim(${objects.title}))`
	const field = matchOn.slice('metadata.'.length)
	return sql<string>`lower(btrim(${objects.metadata} ->> ${field}))`
}

const MATCH_LOOKUP_CHUNK = 500

/**
 * Map each key to the id of an existing object of `objectType` whose `matchOn`
 * value normalizes to it. When the workspace already holds several objects
 * with the same key, the oldest one wins.
 */
async function findExistingByKey(
	...
	const keyExpr = matchKeyExpr(matchOn)
	for (let i = 0; i < keys.length; i += MATCH_LOOKUP_CHUNK) {
		const chunk = keys.slice(i, i + MATCH_LOOKUP_CHUNK)
		const rows = await db
			.select({ id: objects.id, key: keyExpr })
			.from(objects)
			.where(
				and(
					eq(objects.workspaceId, workspaceId),
					eq(objects.type, objectType),
					inArray(keyExpr, chunk),
				),
			)
			.orderBy(asc(objects.createdAt))
		for (const row of rows) {
			if (row.key && !found.has(row.key)) found.set(row.key, row.id)
		}
	}
	return found
}
...
export async function executeImport(
	...
	for (const [tmIndex, typeMapping] of mapping.typeMappings.entries()) {
		...
		for (const row of rows) {
			...
			const key = mapped ? matchKeyFor(mapped, matchOn) : undefined
			if (key) keys.add(key)
		}
		knownByKey.set(
			tmIndex,
			keys.size > 0
				? await findExistingByKey(db, workspaceId, typeMapping.objectType, matchOn, [...keys])
				: new Map(),
		)
	}
