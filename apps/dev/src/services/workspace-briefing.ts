...
// Commitments are ordered by how loud a signal they carry: `breached`
// (floor already broken) first, then `at-risk`, then `holding`
// (steady-state). Mirrors the bet convention where terminal transitions
// surface ahead of lifecycle noise. The composer's per-type grouping
// requires at-risk and breached to appear ahead of holding.
...
export async function renderWorkspaceBriefing(
	...
	const commitmentLabel = displayNames.commitment ?? 'Commitment'
	...
	// `COMMITMENT_ATTENTION_STATUSES` stays the single source of truth
	// shared with the unread-feed join in routes/subscriptions.ts — no
	...
	// `at-risk` row win one of the MAX_COMMITMENTS slots over an older
	// `breached` one, because the in-memory 3-tier sort below only re-sorts
	// whatever already survived the LIMIT. `commitmentBreachedPriority`
	// breaks the tie within the attention-worthy tier so breached rows are
	// never truncated in favor of at-risk ones.
	...
	const [activeBets, pausedBets, closedBets, openInsights, commitments] = await Promise.all([
		...
		// Commitments — polymorphic filter on `type='commitment'`, never
		// `metadata_eq`, so the week-4 kill signal on the parent bet can read
		// this as usage of the primitive rather than a bespoke query path.
		db
			.select()
			.from(objects)
	...
	// Silent when empty — matches the paused-bets pattern. An empty
	// commitment set (the day-1 state before any bet has graduated) should
	// not shout at the reader with a placeholder line.
