...
webhookApp.post('/slack-interactive', async (c) => {
	...
	// `webhook_deliveries.workspace_id` is `notNull` but carries no FK, so
	// we share a fixed sentinel across every interactive delivery — Slack's
	// `trigger_id` is already globally unique per click, so collapsing the
	// scope to a single row per click is what we want.
