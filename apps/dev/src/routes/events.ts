...
// Comment-triggered session dispatch (@-mention + thread-reply) lives in the
// trigger-runner's `CommentDispatchRunner` subscriber — see
// `services/trigger-runner.ts`. Route stays lean, one code path, one log
// stream (spec §Solution sketch; Magnus consolidation comments
// 515287/516012/516065).
...
app.openapi(createCommentRoute, (async (c) => {
	...
	// Case-1a mention dispatch (agent session per @mentioned agent) is now
	// owned by `CommentDispatchRunner` in `services/trigger-runner.ts`, which
	// picks up the `commented` event over PG NOTIFY and spawns sessions with
	// `triggerSource: 'comment_fallback'` + `sourceCommentEventId`. Removed
	// from this route so the API stays lean and there is exactly one code
	// path + one log stream for comment→dispatch (spec §Solution sketch;
	// Magnus consolidation lock in comments 515287/516012/516065).
	//
	// Thread-scoped auto-replies (Case 1b, agent-thread reply) are the
	// province of bet 26328c8b and stay in this route for now — the
	// always-a-responder bet's spec explicitly delegates Case 1b to that
	// bet ("this bet does NOT re-implement it"). The 5-in-a-row cap
