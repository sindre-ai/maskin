...
app.post('/', async (c) => {
	...
	if (!workspaceId) {
		return c.json(
			createApiError(
				...
				undefined,
				'The Email MCP route is workspace-scoped — include the workspace id in the request headers.',
			),
			400,
		)
	}
	...
	// carries a full initialize + tools/list + tools/call cycle end-to-end
	// so there's no cross-request state to leak between workspaces.
