...
test.describe('Briefing page — /$workspaceId/briefing', () => {
	for (const viewport of SHIP_GATE_VIEWPORTS) {
		test(`renders the ## Loops section for seeded loops at ${viewport.label}`, async ({
			page,
			account,
		}) => {
			await page.setViewportSize({ width: viewport.width, height: viewport.height })

			// Seed one loop per health state so the priority ordering is exercised.
			...
			// The ## Loops markdown heading renders as an <h2> after the composer runs.
	...
	test('stays silent (no ## Loops heading) when no loops are seeded', async ({ page, account }) => {
		...
		await expect(page.getByRole('heading', { name: 'Loops', level: 2 })).toHaveCount(0)
	})
})
