...
// count beside the Marketplace title. N is the package count from the same
...
test.describe('Marketplace catalog count', () => {
	for (const viewport of SHIP_GATE_VIEWPORTS) {
		test(`renders next to the title and matches the "All" filter count at ${viewport.label}`, async ({
			...
			const catalogSize = Number(((await count.textContent()) ?? '').match(/^(\d+)/)?.[1])
			expect(Number.isFinite(catalogSize) && catalogSize > 0).toBe(true)

			// Same-source check: the sidebar (≥md) and mobile chip strip both
			// render an "All N" from the same catalog-packages payload that
			...
			expect(allCount).toBe(catalogSize)
