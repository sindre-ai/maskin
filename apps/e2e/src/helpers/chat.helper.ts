...
export interface InstallChatOpts {
	workspaceId: string
	humanActorId: string
...
const DEFAULT_STREAM_EVENT = {
	...
	message: {
		id: 'msg-e2e-1',
		content: [{ type: 'text', text: 'Hello from Workspace Coach' }],
	},
}

/**
 * Installs Playwright route mocks for the chat-specific slice of the API
 ...
 */
export async function installChatMocks(page: Page, opts: InstallChatOpts): Promise<ChatMockState> {
	const sessionId = opts.sessionId ?? 'e2e-chat-session'
	const agentActorId = opts.agentActorId ?? 'e2e-agent-actor'
	...
	// Agent, plus any extra agents the test seeded.
	...
	await page.route('**/api/sessions', async (route: Route) => {
		...
		await route.fulfill({
			status: 201,
			contentType: 'application/json',
			body: JSON.stringify(buildSessionResponse(sessionId, agentActorId, opts.workspaceId)),
		})
	})
...
function buildSessionResponse(sessionId: string, agentActorId: string, workspaceId: string) {
