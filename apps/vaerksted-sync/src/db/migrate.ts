...
// Mirrors `apps/vaerksted-auth/src/db/migrate.ts`'s runner shape (itself
// mirroring `packages/db/src/migrate.ts`), pointed at this app's own,
// separate migration folder (`apps/vaerksted-sync/drizzle/`). Statements are
// split and run individually rather than as one batch — see
...
// package root (apps/vaerksted-sync/drizzle/) — see drizzle.config.ts's
// comment on why `out` is './drizzle' relative to package root, not this file.
const __dirname = dirname(fileURLToPath(import.meta.url))
const migrationsDir = join(__dirname, '..', '..', 'drizzle')

const databaseUrl = process.env.VAERKSTED_SYNC_DATABASE_URL
if (!databaseUrl) {
	console.error('VAERKSTED_SYNC_DATABASE_URL is not set — cannot run migrations')
	process.exit(1)
}
