...
- **App icon never shows on the home screen, even after deleting and reinstalling** — `ios:init`'s generated `src-tauri/gen/apple/project.yml` doesn't set `ASSETCATALOG_COMPILER_APPICON_NAME`, so Xcode never compiles `Assets.xcassets` into the app's icon set or injects `CFBundleIconName`/`CFBundleIcons` into `Info.plist` — the app installs and runs fine, just with no icon reference at all (not a caching issue, so reinstalling doesn't help). `src-tauri/gen/apple` is git-ignored and fully regenerated by `ios:init`, so this needs to be reapplied any time that command is re-run against a clean/deleted `gen/apple`:
  1. Add `ASSETCATALOG_COMPILER_APPICON_NAME: AppIcon` under `targets.maskin-mobile_iOS.settings.base` in `src-tauri/gen/apple/project.yml`.
  2. Add `ASSETCATALOG_COMPILER_APPICON_NAME = AppIcon;` to the app target's two `XCBuildConfiguration` entries (debug/release — the ones with `INFOPLIST_FILE`/`PRODUCT_BUNDLE_IDENTIFIER`) in `src-tauri/gen/apple/maskin-mobile.xcodeproj/project.pbxproj`, or just re-run `xcodegen generate` from `src-tauri/gen/apple` after step 1 to regenerate the `.pbxproj` from `project.yml`.
  3. Rebuild/reinstall — `actool`/`CompileAssetCatalog` should now appear in the build log, and the icon PNGs get emplaced at the app bundle root.
...
2. **AppDelegate token capture.** The APNs device token is delivered to `application(_:didRegisterForRemoteNotificationsWithDeviceToken:)` on the app delegate. The template at `src-tauri/ios/AppDelegate.swift.template` now includes this hook alongside the tap-deep-link hooks — paste the whole template into `src-tauri/gen/apple/Sources/AppDelegate.swift` after `ios:init`. It hex-encodes the token, derives the APNs environment from `#if DEBUG` (sandbox / production), and hands both to the Rust extern `maskin_apns_device_token_received`. The Rust bridge stashes the token in a mutex slot and emits an `apns-device-token` Tauri event. The JS listener at `apps/web/src/lib/ios-push-token.ts` (wired at boot in `apps/web/src/main.tsx`) reads both surfaces and PATCHes `/api/apns-tokens`, which upserts a row keyed by token. Registration is unconditional on every launch, so a rotated token heals on the next foreground.
