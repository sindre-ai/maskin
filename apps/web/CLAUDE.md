...
- Settings section labels ("WORKSPACE NAME", "APPEARANCE", "PRIVACY & DATA", "PAYMENT METHOD", …) ship as the `.settings-label` utility — 11px sans, 600, 0.04em tracking, uppercase, muted-foreground. Settings is the only surface that uses it; everywhere else the micro label is `.eyebrow`.
