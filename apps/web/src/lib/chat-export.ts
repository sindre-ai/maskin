...
/**
 * Serialises the chat transcript to a markdown document suitable for copy /
 ...
 */
export function formatChatMarkdown(events: ChatEvent[], ctx: ChatExportContext): string {
...
function renderAttachment(attachment: UserAttachmentView, ctx: ChatExportContext): string {
...
export function buildChatExportFilename(agentName: string, date: Date = new Date()): string {
	const slug =
		agentName
			.trim()
			.toLowerCase()
