...
/** How many selected objects a single "Ask an agent" hand-off carries. */
const ASK_AGENT_REFERENCE_LIMIT = 12
...
function ObjectsPage() {
	...
	// The rows the active view is actually showing — `Select all` fills to this,
	// not to everything loaded, so it can never select a row the current filters
	// have hidden.
	...
	const handleSelectAll = useCallback(() => {
		...
		for (const object of selectableObjects) next[object.id] = true
		setRowSelection(next)
	}, [selectableObjects])

	// Mockup 5808–5813: hand the selection to a new chat as references rather
	// than acting on it here — the agent reads them and you say what you want.
	// Capped at 12, the same ceiling the mockup uses, so a select-all doesn't
	// push a thousand ids through the URL.
	const handleAskAgent = useCallback(() => {
		if (selectedIds.length === 0) return
		const ids = selectedIds.slice(0, ASK_AGENT_REFERENCE_LIMIT)
		if (selectedIds.length > ids.length) {
			toast.info(`Referencing the first ${ids.length} of ${selectedIds.length} selected`)
		}
		clearSelection()
		navigate({
			to: '/$workspaceId/chats/new',
			params: { workspaceId },
			search: { objectIds: ids.join(',') },
		})
	}, [selectedIds, clearSelection, navigate, workspaceId])
	...
	// <h1> — the mockup puts the type tabs beside "Objects", not out beyond the
	// search field. PageHeader's effect deps are `[titleTabs]`, so a fresh node
	// every render would re-set context state on every pass; memoised for that.
