...
describe('useInstallCatalogItem', () => {
	it('invalidates workspaceSkills and integrations caches on success', async () => {
		vi.mocked(api.catalogItems.install).mockResolvedValue({
			id: 'item-1',
			item_type: 'actor',
			name: 'Test Actor',
		})
		const { queryClient, wrapper } = makeWrapper()
		const { result } = renderHook(() => useInstallCatalogItem(workspaceId), { wrapper })
	...
	it('also invalidates actors, triggers, and catalogItems on success', async () => {
		vi.mocked(api.catalogItems.install).mockResolvedValue({
			id: 'item-1',
			item_type: 'actor',
			name: 'Test Actor',
...
describe('useUninstallCatalogItem', () => {
	it('invalidates workspaceSkills and integrations caches on success', async () => {
		vi.mocked(api.catalogItems.uninstall).mockResolvedValue({ deleted: true })
		const { queryClient, wrapper } = makeWrapper()
		const { result } = renderHook(() => useUninstallCatalogItem(workspaceId), { wrapper })
