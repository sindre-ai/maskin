...
describe('v1 taxonomy helpers', () => {
	...
	it('fyp_briefing_read dedupes per briefing per session', () => {
		const capture = captureSpy()
		trackFypBriefingRead({ workspace_id: 'ws-1', briefing_id: 'brief-a' })
		trackFypBriefingRead({ workspace_id: 'ws-1', briefing_id: 'brief-a' })
		trackFypBriefingRead({ workspace_id: 'ws-1', briefing_id: 'brief-b' })
		const readCalls = capture.mock.calls.filter(([name]) => name === 'fyp_briefing_read')
		expect(readCalls).toHaveLength(2)
		expect(readCalls[0][1]).toEqual({ workspace_id: 'ws-1', briefing_id: 'brief-a' })
		expect(readCalls[1][1]).toEqual({ workspace_id: 'ws-1', briefing_id: 'brief-b' })
	})

	it('fyp_briefing_audio_played dedupes per briefing per session', () => {
		const capture = captureSpy()
		trackFypBriefingAudioPlayed({ workspace_id: 'ws-1', briefing_id: 'brief-a' })
		trackFypBriefingAudioPlayed({ workspace_id: 'ws-1', briefing_id: 'brief-a' })
		trackFypBriefingAudioPlayed({ workspace_id: 'ws-1', briefing_id: 'brief-b' })
		const audioCalls = capture.mock.calls.filter(([name]) => name === 'fyp_briefing_audio_played')
		expect(audioCalls).toHaveLength(2)
		expect(audioCalls[0][1]).toEqual({ workspace_id: 'ws-1', briefing_id: 'brief-a' })
		expect(audioCalls[1][1]).toEqual({ workspace_id: 'ws-1', briefing_id: 'brief-b' })
	})
})
