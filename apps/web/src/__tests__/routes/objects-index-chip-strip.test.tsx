...
const searchState = vi.hoisted(() => ({
	current: {
		...
		// Quick filters that narrow the loaded rows client-side.
		fresh: undefined as 1 | undefined,
		starred: undefined as 1 | undefined,
	},
}))
...
describe('ObjectsPage bulk edit', () => {
	it('fills Select all to every row the current view shows', () => {
		objectsState.rows = [buildRow(1), buildRow(2), buildRow(3)]
		render(<ObjectsPage />)
		...
		expect(bulkBarCapture.lastProps?.selectedCount).toBe(3)
	})

	it('never selects a row the active filters have hidden', () => {
		// `fresh` keeps only rows touched in the last 7 days.
		searchState.current.fresh = 1
		const recent = new Date().toISOString()
		objectsState.rows = [
			buildRow(1, { updatedAt: recent }),
			buildRow(2, { updatedAt: '2020-01-01T00:00:00.000Z' }),
		]
		render(<ObjectsPage />)
		...
		expect(bulkBarCapture.lastProps?.selectedCount).toBe(1)
	})

	it('hands at most twelve references to a new chat from Ask an agent', () => {
		objectsState.rows = Array.from({ length: 15 }, (_, i) => buildRow(i + 1))
		render(<ObjectsPage />)

		act(() => {
			;(bulkBarCapture.lastProps?.onSelectAll as () => void)()
		})
		expect(bulkBarCapture.lastProps?.selectedCount).toBe(15)
		...
		expect(navigateMock).toHaveBeenCalledWith({
			...
			search: { objectIds: Array.from({ length: 12 }, (_, i) => `obj-${i + 1}`).join(',') },
		})
		// The hand-off clears the selection — the work moved to the chat.
		expect(bulkBarCapture.lastProps?.selectedCount).toBe(0)
	})
})
