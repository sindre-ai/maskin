...
// Which create action the split New button's primary (left) half runs —
// keyed off the current screen (`header.tsx`'s `routeConfig[...].primary`).
// Screens that create nothing in particular fall through to 'chat'.
export type PrimaryKind = 'chat' | 'object' | 'agent' | 'loop'
...
interface NewMenuProps {
	...
	// Defaults to 'chat' for screens with no stronger opinion (For You, chats).
	primaryKind?: PrimaryKind
}

// The split "+ New" control used everywhere (global header, For You page) so
// the trigger and dropdown contents can never drift apart between pages. The
// primary half runs the screen's own create action directly; the chevron half
// opens the full menu of every other way to start something.
export function NewMenu({ onNewChat, hideObjectSection, primaryKind = 'chat' }: NewMenuProps) {
	...
	const runPrimary = () => {
		...
		if (primaryKind === 'object') {
			setCreateConfig({ type: 'object' })
			return
		}
		setCreateConfig({ type: primaryKind })
	}
		...
		<>
			<div className="flex items-center">
				<Button
					size="sm"
					onClick={runPrimary}
					aria-label={`New ${PRIMARY_LABEL[primaryKind].toLowerCase()}`}
					className="h-7 gap-1 rounded-r-none px-2"
				>
					<Plus size={14} aria-hidden />
					<span className="hidden sm:inline">New {PRIMARY_LABEL[primaryKind]}</span>
				</Button>
				<DropdownMenu>
					<DropdownMenuTrigger asChild>
						<Button
							...
							className="h-7 w-6 shrink-0 rounded-l-none border-l border-l-primary-foreground/20 px-0"
						>
