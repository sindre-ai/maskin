// Pre-v2 Objects surface, rendered when the `new-design` feature flag is off.
// Governed by the `new-design` flag; this directory dies with the flag.

import { ActionBanner } from '@/components/activity/action-banner'
...
import {
	...
	SIDEBAR_WIDTH,
} from '@/components/objects/properties-sidebar-provider'
import { VerifiedChip, isKnowledgeAuthorWrite } from '@/components/objects/verified-chip'
...
export function LegacyObjectDocument({ object }: { object: ObjectResponse }) {
			...
			onSuccess: () => {
				navigate({
					...
					search: (prev) => ({
						...
						status: prev.status,
						driver: prev.driver,
						// Shared with the v2 route's `validateSearch` — the URL schema is
						// data, not visual layer, so it does not fork behind the flag.
