...
// A second, right-side sidebar instance lives in its own context so it can be
// mounted alongside the existing left-nav `SidebarProvider` without either
// clobbering the other's state or keyboard shortcut. Toggled with ⌘/Ctrl+I;
// no cookie is written — right-side state is externally controlled per the
// object-detail sidebar architecture decision.
const SidebarRightContext = React.createContext<SidebarContextProps | null>(null)
...
function SidebarRightProvider({
	...
	return (
		<SidebarRightContext.Provider value={contextValue}>{children}</SidebarRightContext.Provider>
	)
}
...
>(
	(
		...
		if (!ctx) {
			throw new Error('Sidebar must be used within a SidebarProvider or SidebarRightProvider.')
		}
...
>(({ side = 'left', className, onClick, ...props }, ref) => {
	...
	const ctx = side === 'right' && rightCtx ? rightCtx : leftCtx
	if (!ctx) {
		throw new Error('SidebarTrigger must be used within a SidebarProvider or SidebarRightProvider.')
	}
...
>(({ side = 'left', className, ...props }, ref) => {
	...
	const ctx = side === 'right' && rightCtx ? rightCtx : leftCtx
	if (!ctx) {
		throw new Error('SidebarRail must be used within a SidebarProvider or SidebarRightProvider.')
	}
	const { toggleSidebar } = ctx
