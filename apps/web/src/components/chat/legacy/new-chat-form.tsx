...
/**
 * Pre-v4 new-chat screen (bet/bdda1c1e-chats-v4-polish). Vendored verbatim from
 * the pre-bet `chats/new.tsx` body and rendered by that route when the
 * `chats-v4-polish` umbrella flag OR its `.new_chat` sub-flag is off — see
 * `.claude/rules/feature-flags.md` and `chats/new.tsx` for the boundary. The v4
 * rewrite replaces the header's picker popover with a chip input + RECENT
 * dropdown and adds the analytics session_started event; neither is here, which
 * is the point: this file is the rollback, so it must not drift. Retire by
 * deleting this directory, the boundary in `chats/new.tsx`, and the flag ids in
 * `apps/dev/src/lib/feature-flags.ts`.
 */

export interface LegacyNewChatSearch {
	agentId?: string
	agentName?: string
...
export function LegacyNewChatForm({ search }: { search: LegacyNewChatSearch }) {
