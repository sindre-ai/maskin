// Pre-v2 shell, rendered when the `new-design` feature flag is off.
// Restored verbatim from origin/main before PR #1404 rewrote the shell in
// place. Delete this whole directory when the flag is retired.
...
export function LegacyHeader() {
	...
	// On origin/main the For You route rendered its own NewMenu, so this header
	// suppressed a duplicate. PR #1404 removed that menu from ForYouHeaderActions
	// (the v2 header owns it now), and that component is shared by both shells —
	// so under the legacy shell this header must render the menu on For You too,
	// or the route loses its New button entirely.
