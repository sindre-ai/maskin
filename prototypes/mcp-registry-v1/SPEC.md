...
- **New patterns:** three, each with a one-line "why the existing patterns fail this job".
  ...
  - **`<HealthPulse>`** — a status dot + label combo tuned for the three MCP states (healthy / auth-failed / server-down), with an inline action slot for the fix (`Reconnect` on auth-failed, `Retry now` on server-down). **Why new:** the app's current `.status-dot` pattern (from Keychain) is stateful but action-less. Health needs the fix legally next to the state — if the user has to click through to reach the reconnect button, the Lost criterion (>20% first-use failures = anti-credibility) manifests as silent frustration instead of visible pain. This is the single design decision that pulls the Lost criterion out of the UI.
...
- **Success:** Fix-click routes into Reconnect (Keychain OAuth wizard) or Retry-now (in-place ping + spinner + toast on recovery).
...
- **Motion:** All colour + border transitions use `--duration-150 var(--ease-standard)`. Modal + sheet enter/exit use `--duration-slide` (300 ms) `var(--ease-emphasized)`. Health pulse dot is a 2.5-s ease-in-out opacity loop (only on healthy state — auth-failed/server-down states are static so they visually differentiate). All animations honour `prefers-reduced-motion` (already global via `app.css`).
...
- **Accessibility:**
  ...
  - `<HealthPulse>` uses `role="status"` and includes the fix action in the same live region so screen-readers announce state + fix together.
...
1. **Curation source of truth — Maskin-hosted registry or `registry.modelcontextprotocol.io` fork?** The prototype's sync-status eyebrow says "registry.modelcontextprotocol.io · synced 4 min ago" — assumes we index the official registry and layer our grade + AAA-verification tier on top. Alternative: Maskin publishes its own registry (curated set only, no sync). The first is faster to ship and matches the SEO research's "buyer workflow" pattern. The second gives us more editorial control. Ask **CPO** which shape wins v1. Prototype currently mocks option 1.
2. **AAA-verification tier bar.** The prototype uses AAA for "Maskin has tested this MCP against a live workspace this week" and Official for "the vendor publishes it themselves." Do those tests need to be automated (a nightly job hitting each MCP with a canary loop) or is manual weekly QA enough to earn AAA? Ask **Architect** — the answer changes whether the trust grade needs a build pipeline or just a docs page.
3. **BYO OAuth 2.1 in v1 — or just Bearer / API-key / None?** The prototype shows OAuth 2.1 as one of the four BYO auth options, but implementing it well (PKCE, dynamic client registration, refresh handling for arbitrary services) is a substantial slice. The Keychain SPEC's OAuth flow is scoped to services where Maskin owns the OAuth client OR the user provides their own OAuth app. BYO here means the *MCP endpoint* speaks OAuth 2.1 — a slightly different (and simpler) contract. Ask **Architect** whether BYO OAuth 2.1 for arbitrary MCP endpoints ships in v1 or slips.
4. **Per-loop activation UX inside the Rails-like loop builder — is the current loop-builder amenable to inline panels?** The prototype assumes the loop step canvas is a vertical stack of expandable step cards, and the MCP activation panel sits inside step 2's card body. If the current loop-builder is a graph-canvas (nodes + edges), the panel affordance needs a different home — probably a right-hand inspector when a step is selected. Ask **Developer** which shape the current loop builder actually is; if graph-canvas, this prototype's step-card model needs a v2 pass before Planner breaks tasks.
5. **First-use failure rate — do we have the instrumentation today?** The Health view's sparkline against the 20% Lost ceiling depends on us being able to distinguish first-use from repeated-use tool calls, per credential, per MCP. If the events table doesn't already carry `is_first_use`, this needs an Architect scope call before Planner. Ask **Architect**.
6. **Auth-expired detection latency.** Prototype claims "detected 4 h ago" — implying we background-check auth freshness. If we only learn a token is expired when a loop actually tries to use it, the Health view lags reality by hours (or days for infrequent loops). Worth an Architect call on whether we run a lightweight `whoami`-per-MCP daily check, or fall back to lazy detection. Prototype currently assumes proactive.
