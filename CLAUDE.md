...
- **When a PR changes the auth model, workspace-scoping pattern, or the declared threat model, refresh `.deepsec/data/maskin/INFO.md` in the same PR.** That file is the only repo-specific context DeepSec's AI review gets — if it lags behind reality, scans start missing real bugs or raising known-good patterns as findings. Concretely: any diff to `packages/auth/**`, `apps/dev/src/lib/workspace-auth.ts`, `apps/dev/src/routes/actors.ts`, `SECURITY.md`, or `.maskin/protected-paths.yml` should either update `INFO.md` in the same commit or state in the PR description why no update is needed.
